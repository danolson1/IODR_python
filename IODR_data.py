...
import pandas as pd
import pytz # python timzeone module
...
# helper methods are outside of the IODR class
...
def get_data(chID, start_date, end_date = dt.datetime.now().strftime("%Y-%m-%d %H:%M")):
    """
    ...
    logger.debug(r.content.decode('utf-8')[0:200])
...
class IODR:
    def __init__(self, device_number):
        ...
        self.chID = chIDs[device_number-1]
        self.readAPIkey = readAPIkeys[device_number-1]
  
        
    def get_all_data(self, start_date, end_date):
        """
        Get all requested data from Thingspeak, for a specific IODR object, making multiple requests if needed
